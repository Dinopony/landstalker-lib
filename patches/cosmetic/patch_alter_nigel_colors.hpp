...
/**
 * This patch alters Nigel colors, while also fixing the shade on his sword being changed when HUD palette is changed.
 */
class PatchAlterNigelColors : public GamePatch
{
private:
    Color _light_color;
    Color _dark_color;
...
public:
    explicit PatchAlterNigelColors(const std::pair<Color, Color>& nigel_colors) :
        _light_color    (nigel_colors.first),
        _dark_color     (nigel_colors.second)
    {}

    void alter_rom(md::ROM& rom) override
    {
        set_nigel_color(rom, 9, _light_color);
        set_nigel_color(rom, 10, _dark_color);
...
private:
    ...
    static constexpr uint8_t COLOR_SWORD_WRONG = 9;
    static constexpr uint8_t COLOR_SWORD_REPLACEMENT = 6;
