...
const std::map<uint8_t, uint8_t> DEFAULT_PALETTES = {
...
const std::set<uint8_t> LIFTABLE_TYPES = {
    ENTITY_CRATE, ENTITY_VASE, ENTITY_BUTTON_CUBE
};

const std::set<uint8_t> FIGHTABLE_TYPES = {
    ENTITY_SMALL_GRAY_SPIKEBALL,
    ENTITY_LARGE_GRAY_SPIKEBALL,
    ENTITY_SACRED_TREE
};

const std::set<uint8_t> ENEMY_TYPES = {
...
Entity* Entity::from_rom(const md::ROM& rom, uint32_t addr, Map* map)
{
    ...
    if(points_at_itself)
        attrs.entity_to_use_tiles_from = entity;
    entity->_attrs = std::move(attrs);
    entity->_map = map;
